...
class TestsIntegerList(unittest.TestCase):
    ...
    def test_insert_method_with_valid_index_and_value_should_work(self):
        data = [1, 3, 4]
        integer_list = IntegerList(*data)
        expected = [1, 2, 3, 4]
    ...
    def test_insert_method_with_invalid_index_should_give_index_error(self):
        data = [1, 3, 4]
        integer_list = IntegerList(*data)

        with self.assertRaises(IndexError) as context:
            integer_list.insert(3, 2)
        self.assertEqual(str(context.exception), "Index is out of range")

    def test_insert_method_with_invalid_data_type_should_give_Value_error(self):
        data = [1, 3, 4]
        integer_list = IntegerList(*data)

        with self.assertRaises(ValueError) as context:
            integer_list.insert(0, 'not an integer')
        self.assertEqual(str(context.exception), "Element is not Integer")

    def test_get_biggest_method_should_return_the_biggest_number(self):
        data = [1, 2, 3]
        integer_list = IntegerList(*data)
    ...
    def test_get_index_should_return_the_index_of_the_given_element(self):
        data = [1, 2, 3]
        integer_list = IntegerList(*data)
