...
SUPyF2 P.-F.-Exam/9 August 2019. - 02. Password
...
Comment:
The first one doesn’t start and end with the same amount of '#'
and the count of characters in each group is different than 3.
The second one is correct. The third one uses the wrong '<' and '>'
    ...
    if ">" in a and "<" in a:
        ...
        if len(start) == len(end):
            ...
            for group in middle:
                if len(group) != 3:
                    all_are_three = False
            if all_are_three and len(middle) == 4:
                print(start)
                print(end)
                print(middle)
